...
export function HabitatDirectory({ activeLocation = null, onActiveLocation = () => undefined }: HabitatDirectoryProps) {
  return <div className="divide-y divide-border">
    {locations.map((location) => <Link key={location.slug} to="/colony/$slug" params={{ slug: location.slug }}
      ...
      <div className="min-w-0 flex-1"><div className="flex items-center justify-between gap-2"><strong className={cn("text-xs font-semibold text-foreground transition-colors duration-200 group-hover:text-accent", activeLocation === location.slug && "text-accent")}>{location.name}</strong><span className="font-mono text-[9px] text-accent">{location.count}</span></div><p className="mt-1 truncate text-[11px] text-muted-foreground">{location.short}</p></div>
    </Link>)}
  </div>;
}