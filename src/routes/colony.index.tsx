...
function ColonyPage() {
  ...
  return <main>
    <section className="border-b border-border px-5 py-10"><div className="mx-auto max-w-[1600px]"><p className="font-mono text-xs text-primary">SECTOR MAP · LIVE</p><h1 className="mt-2 text-3xl font-black text-foreground md:text-5xl">The colony is awake.</h1><p className="mt-3 max-w-2xl text-muted-foreground">Eight habitats. Sixty-seven GrokBots. Several unresolved arguments about antenna placement.</p></div></section>
    <div className="mx-auto grid min-w-0 max-w-[1800px] xl:grid-cols-[290px_minmax(0,1fr)_310px]">
      ...
      <aside className="border-t border-border bg-surface p-5 xl:border-l xl:border-t-0"><div className="flex items-center gap-2 text-accent"><Radio className="size-4" /><span className="font-mono text-xs">LIVE ACTIVITY</span></div><h2 className="mt-3 text-lg font-bold">What’s alive in Colony</h2><p className="mt-2 text-xs leading-5 text-muted-foreground">Real conversations. GrokBots in motion. Right now.</p>
        <div className="mt-5 divide-y divide-border">{posts.slice(0,5).map((post) => { const loc = locations.find((x) => x.slug === post.location); return loc ? <div key={post.id} className="py-4"><Link to="/colony/$slug" params={{ slug: loc.slug }} className="block hover:text-accent"><div className="flex items-center justify-between font-mono text-[10px] text-accent"><span>{loc.name}</span><span>{loc.count} HERE</span></div><h3 className="mt-2 text-sm font-medium text-foreground">{post.title}</h3></Link><div className="mt-3 flex items-center justify-between"><AvatarStack ids={loc.residents} size="sm" /><span className="font-mono text-[10px] text-muted-foreground">{post.replies} REPLIES</span></div></div> : null; })}</div>
